...
(() => {
  ...
  historyBtn.addEventListener('click', () => { document.querySelector('section:not(#history-screen)').style.display='none'; document.getElementById('history-screen').style.display='block'; renderHistory(); });
  backBtn.addEventListener('click', () => { document.getElementById('history-screen').style.display='none'; document.querySelector('section:not(#history-screen)').style.display='block'; });
...
// Navegação para histórico de vendas
...
function saveHistory() {
  ...
  historyArr.push({ timestamp: new Date().toISOString(), items: sale.items.slice(), total: sale.total });
...
function renderHistory() {
  ...
  historyArr = historyArr.filter(rec => {
    let ok = true;
    if (start) ok = ok && new Date(rec.timestamp) >= new Date(start);
    if (end) ok = ok && new Date(rec.timestamp) <= new Date(end);
    if (search) {
      ok = ok && rec.items.some(i => i.name.toLowerCase().includes(search));
    }
    return ok;
  });
  historyArr.forEach(rec => {
    const tr = document.createElement('tr');
    const date = new Date(rec.timestamp).toLocaleString();
    const itemsText = rec.items.map(i => `${i.name} (${i.qty})`).join(', ');
    tr.innerHTML = `<td>${date}</td><td>${itemsText}</td><td>${formatBRL(rec.total)}</td>`;
...
resetFiltersBtn.addEventListener('click', () => { filterStart.value=''; filterEnd.value=''; filterSearch.value=''; renderHistory(); });
